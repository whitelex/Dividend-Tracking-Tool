...
const App: React.FC = () => {
  ...
  const [portfolio, setPortfolio] = useState<PortfolioState>(() => {
    const saved = localStorage.getItem('divi_track_data_v2');
  ...
  useEffect(() => {
    localStorage.setItem('divi_track_data_v2', JSON.stringify(portfolio));
  }, [portfolio]);
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex space-x-3">
             <button 
              ...
              className="bg-white text-indigo-700 px-4 py-2 rounded-xl font-bold flex items-center shadow-md hover:bg-indigo-50 transition-all active:scale-95"
            >
              <Plus className="h-4 w-4 mr-2" /> Add Purchase
            </button>
            <button 
              onClick={() => setIsDivModalOpen(true)}
              className="bg-indigo-600 text-white border border-indigo-500 px-4 py-2 rounded-xl font-bold flex items-center hover:bg-indigo-500 transition-all active:scale-95"
            >
                    ...
                    {stockStats.length > 0 ? stockStats.map(stock => (
                          ...
                          <tr>
                            <td colSpan={7} className="px-6 py-4 bg-slate-50/50">
                              <div className="pl-10 space-y-3">
                                ...
                                <div className="grid grid-cols-4 gap-4 bg-white p-4 rounded-2xl border border-slate-200 shadow-sm">
                      ...
                      <tr>
                        <td colSpan={7} className="px-6 py-20 text-center text-slate-300 italic">
                          ...
                          <p className="text-sm">Click "Add Purchase" to track your first dividend asset.</p>
                        </td>
                      </tr>
                    )}
                  </tbody>
